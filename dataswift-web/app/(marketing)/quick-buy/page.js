...
function GuestBuyContent() {
      ...
      return (
        <div className="min-h-[60vh] flex items-center justify-center">
          <div className="text-center">
            ...
            <h1 className="text-xl font-bold text-secondary">Processing your order...</h1>
            <p className="text-secondary/40 text-sm mt-2">Please wait while we confirm your payment.</p>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center max-w-sm">
          <div className={`w-16 h-16 ${isSuccess ? 'bg-success/10' : 'bg-red-50'} rounded-2xl flex items-center justify-center mx-auto mb-4`}>
            {isSuccess ? <Check className="w-8 h-8 text-success" /> : <AlertCircle className="w-8 h-8 text-red-500" />}
          </div>
          <h1 className="text-xl font-bold text-secondary">
            {isSuccess ? 'Payment Received!' : 'Something went wrong'}
          </h1>
          <p className="text-secondary/40 text-sm mt-2 mb-2">
            {isSuccess
          ...
          {isSuccess && status !== 'completed' && (
            <p className="text-xs text-secondary/30 mb-6">Data is usually delivered within 2 minutes.</p>
          )}
          <div className="flex gap-3 justify-center mt-6">
            ...
            <Link href="/sign-up">
              <button className="px-6 py-3 border-2 border-secondary/10 text-secondary font-bold text-sm rounded-xl hover:border-secondary/20 transition-colors">
                Create account
              </button>
            </Link>
          </div>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10 space-y-8">
      {/* Header */}
      <div className="text-center">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-primary/10 text-primary text-xs font-bold rounded-full mb-4">
      ...
      {/* Step 1: Network */}
      <div>
        <h2 className="font-bold text-sm text-secondary/50 mb-3">1. Select network</h2>
            ...
            <button
              ...
              className={`p-4 rounded-2xl border-2 transition-all text-center ${
                selectedNetwork === net.id
                  ...
                  : 'border-secondary/[0.06] hover:border-secondary/20 bg-white'
              }`}
            >
      ...
      {selectedNetwork && (
        <div>
          <h2 className="font-bold text-sm text-secondary/50 mb-3">2. Choose a bundle</h2>
          {loadingPackages ? (
            <div className="flex items-center justify-center py-8">
              <Loader2 className="w-6 h-6 text-primary animate-spin" />
            </div>
          ) : packages.length === 0 ? (
            <div className="bg-white rounded-2xl border border-secondary/[0.06] p-6 text-center">
      ...
      {selectedBundle && (
        <div>
          <h2 className="font-bold text-sm text-secondary/50 mb-3">3. Enter details & pay</h2>
          <div className="bg-white rounded-2xl border border-secondary/[0.06] p-6 space-y-4">
            {/* Phone */}
            <div>
              <label className="block text-xs font-semibold text-secondary/50 mb-1.5">Recipient phone number</label>
              <div className="flex items-center gap-2 border rounded-xl px-4 py-3 border-secondary/10 focus-within:border-primary transition-colors">
                ...
                <input
                  ...
                  className="flex-1 outline-none text-sm font-medium bg-transparent text-secondary"
                />
              </div>
            </div>
            ...
            {/* Email */}
            <div>
              <label className="block text-xs font-semibold text-secondary/50 mb-1.5">Your email (for payment receipt)</label>
              <div className="flex items-center gap-2 border rounded-xl px-4 py-3 border-secondary/10 focus-within:border-primary transition-colors">
            ...
            <div className="bg-background rounded-xl p-4 space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-secondary">Mobile Money</span>
              </div>
            ...
            <button
              onClick={handleBuy}
              disabled={buying}
              className="w-full py-3.5 rounded-xl font-bold text-white flex items-center justify-center gap-2 bg-primary hover:opacity-90 transition-opacity disabled:opacity-60"
            >
              {buying ? (
      ...
      {/* Sign up nudge */}
      <div className="text-center text-sm text-secondary/40 pb-8">
